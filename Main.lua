...
MainFrame.Size = UDim2.new(0, 260, 0, 430) -- Ukuran disesuaikan
MainFrame.Active = true
MainFrame.Draggable = true

Title.Parent = MainFrame
Title.Text = "ADI MENU PRO V5"
...
-- --- 2. FITUR WALK SPEED (SLIDER + TYPING) ---
...
SliderBg.Position = UDim2.new(0.1, 0, 0, 70)
...
SpeedInput.Position = UDim2.new(0.3, 0, 0, 85)
...
WhBtn.Position = UDim2.new(0.1, 0, 0, 120)
...
WhBtn.MouseButton1Click:Connect(function()
    for _, p in pairs(players:GetPlayers()) do
        if p ~= lp and p.Character then
            ...
            local function checkTools(loc)
                for _, i in pairs(loc:GetChildren()) do
                    if i:IsA("Tool") and (i.Name:lower():find("knife") or i.Name:lower():find("weapon")) then return true end
                end
                return false
            end
            if checkTools(p.Backpack) or (p.Character and checkTools(p.Character)) then isKiller = true end
...
-- --- 4. FITUR KILLER HITBOX (LOGIC) ---
local HitboxLabel = Instance.new("TextLabel", MainFrame)
HitboxLabel.Text = "Hitbox Extender: 2 (100%)"
...
HitboxInput.Position = UDim2.new(0.1, 0, 0, 185)
...
ScBtn.Position = UDim2.new(0.1, 0, 0, 300)
